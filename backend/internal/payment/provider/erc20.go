...
// Mainnet USDT (ERC20) contract address. Overridable via instance config
// "usdtContract" for testnet (e.g. Sepolia) verification.
const defaultERC20USDTContract = "0xdAC17F958D2ee523a2206206994597C13D831ec7"

// ERC20 implements payment.Provider for self-custodied USDT collection on the
// Ethereum network. It mirrors the TRC20 provider: no upstream webhook,
...
func NewERC20(instanceID string, config map[string]string) (*ERC20, error) {
	...
	if strings.TrimSpace(cfg["usdtContract"]) == "" {
		cfg["usdtContract"] = defaultERC20USDTContract
	}
	return &ERC20{instanceID: instanceID, config: cfg}, nil
}

func (e *ERC20) Name() string        { return "USDT-ERC20" }
func (e *ERC20) ProviderKey() string { return payment.TypeERC20 }
func (e *ERC20) SupportedTypes() []payment.PaymentType {
	return []payment.PaymentType{payment.TypeERC20}
}

// USDTContract returns the configured token contract address.
func (e *ERC20) USDTContract() string { return e.config["usdtContract"] }
