...
// cryptoNetworkTRC20 is the network label stored on user_crypto_addresses.
const cryptoNetworkTRC20 = "TRC20"
...
func (s *CryptoWalletService) InitWallet(ctx context.Context, mnemonic string) (*InitWalletResult, error) {
	...
	res := &InitWalletResult{FeeAddress: feeAddr, Generated: generated, Initialized: true}
	if generated {
...
func (s *CryptoWalletService) EnsureUserAddress(ctx context.Context, userID int64) (*dbent.UserCryptoAddress, error) {
	...
	created, txErr := s.allocateUserAddress(ctx, userID, mgr)
	if txErr != nil {
		// Lost a race: another request created the row first. Re-read.
		if again, qerr := s.entClient.UserCryptoAddress.Query().
			Where(usercryptoaddress.UserID(userID), usercryptoaddress.NetworkEQ(cryptoNetworkTRC20)).
			Only(ctx); qerr == nil {
			return again, nil
		}
		return nil, txErr
...
func (s *CryptoWalletService) allocateUserAddress(ctx context.Context, userID int64, mgr *wallet.Manager) (*dbent.UserCryptoAddress, error) {
	...
	addr, err := mgr.Address(uint32(index))
	if err != nil {
		return nil, err
	}
...
// GetUserAddress returns the user's existing deposit address, or nil if none.
func (s *CryptoWalletService) GetUserAddress(ctx context.Context, userID int64) (*dbent.UserCryptoAddress, error) {
...
func (s *CryptoWalletService) Overview(ctx context.Context) (*WalletOverview, error) {
	...
	// Live balances for the two operational addresses (best-effort).
...
func (s *CryptoWalletService) ListAddresses(ctx context.Context, page, pageSize int) ([]*dbent.UserCryptoAddress, int, error) {
	if page < 1 {
		page = 1
	}
...
func (s *CryptoWalletService) RefreshBalances(ctx context.Context) (int, error) {
	...
	if !ts.instancePresent {
		return 0, infraerrors.BadRequest("NO_TRC20_INSTANCE", "no enabled TRC20 provider instance configured")
	}
	rows, err := s.entClient.UserCryptoAddress.Query().
		Where(usercryptoaddress.NetworkEQ(cryptoNetworkTRC20)).
		All(ctx)
	...
	refreshed := 0
	for _, r := range rows {
		bal, berr := ts.client.TRC20Balance(ctx, r.Address, ts.contract)
		if berr != nil {
			continue
		}
