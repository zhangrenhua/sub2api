...
const (
	...
	AffiliateRebateIncludeSubscriptionDefault = false // 默认订阅订单不参与邀请返利（仅余额充值返利）
)
...
const (
	...
	SettingKeyAffiliateRebateIncludeSubscription = "affiliate_rebate_include_subscription" // 订阅订单是否参与邀请返利（默认 false=仅余额充值）
	...
	SettingKeySupportURL                  = "support_url"                   // 客服链接(Telegram), 空则隐藏悬浮入口
