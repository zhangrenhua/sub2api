...
export const PROVIDER_CONFIG_FIELDS: Record<string, ConfigFieldDef[]> = {
  ...
  // Kyren uses the same Epay protocol as `easypay`, so the config fields mirror it.
  // apiBase: https://api.kyren.top (prod) or https://test-api.kyren.top (sandbox).
  // pkey: Kyren API key (kyren_live_* / kyren_test_*).
  kyren: [
    { key: 'pid', label: 'PID', sensitive: false },
    { key: 'pkey', label: 'API Key', sensitive: true },
    { key: 'apiBase', label: '', sensitive: false, defaultValue: 'https://api.kyren.top' },
  ],
}
