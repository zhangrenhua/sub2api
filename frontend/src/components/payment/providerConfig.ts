...
export const METHOD_ORDER = ['alipay', 'alipay_direct', 'wxpay', 'wxpay_direct', 'stripe', 'airwallex', 'paypal', 'usdt_trc20'] as const
...
export const PROVIDER_CONFIG_FIELDS: Record<string, ConfigFieldDef[]> = {
  ...
  // Self-custodied USDT collection. No upstream gateway secret; wallet
  // mnemonic/sweeping are managed on the dedicated Crypto Wallet page.
  // Plans are priced in CNY; cnyPerUsdt converts to the USDT amount to collect,
  // and minRechargeCny enforces the USDT-only minimum (in CNY).
  usdt_trc20: [
    ...
    { key: 'trongridApiBase', label: 'TronGrid API Base', sensitive: false, defaultValue: 'https://api.trongrid.io' },
    { key: 'trongridGrpcNode', label: 'TronGrid gRPC Node', sensitive: false, defaultValue: 'grpc.trongrid.io:50051' },
    { key: 'trongridApiKey', label: 'TronGrid API Key', sensitive: true, optional: true, clearable: true },
    { key: 'confirmSeconds', label: 'Confirmation Seconds', sensitive: false, optional: true, defaultValue: '60' },
    { key: 'gasTopUpSun', label: 'Gas Top-up (SUN)', sensitive: false, optional: true, defaultValue: '30000000' },
    { key: 'feeLimitSun', label: 'Fee Limit (SUN)', sensitive: false, optional: true, defaultValue: '100000000' },
    { key: 'sweepMinUsdt', label: 'Sweep Min (USDT)', sensitive: false, optional: true, defaultValue: '5' },
  ],
}
